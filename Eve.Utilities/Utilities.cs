...
namespace Eve.Utilities {
	public class Utils {
		...
		public static void RemoveUserFromChannel(string channel, string realname) {
			if (!CheckChannelExists(channel)) return;
			Channel first = IrcBot.V.Channels.FirstOrDefault(e => e.Name == channel);
			if (!first.UserList.Contains(realname)) {
